...
/**
 * @brief Lê dados de um arquivo TSV e bin e armazena os na struct Dados.
 *
 * @param arquivo Nome do arquivo TSV a ser lido na função.
 * @param dados Invocação da struct dados na função.
 */

void ficheiro_tsv_1(char *arquivo, Dados *dados)
{
    if (strcmp(arquivo, "dados.tsv") == 0) //**< Compara as strings, caso sejam iguais entra neste loop */
    {
        ...
        /**
         * @brief Criação de um ciclo while para ler os dados do arquivo TSV e armazená-los na estrutura Dados.
         *
         * @param ficheiro Nome do arquivo TSV a ser lido na função.
         * @param dados Invocação da struct dados na função.
    ...
    else if (strcmp(arquivo, "dados_tsv.bin") == 0) //**< Compara as strings, caso sejam iguais entra neste loop */
    {
        ...
        for (int i = 0; i < 3; i++)
        {
            fread(&dados->n_cliente[i], sizeof(int), 1, ficheiro);      //**< Modo leitura relativamente a dados binários interios */
...
/**
 * @brief Lê dados de um arquivo TSV (tab-separated values) e .bin e armazena os na estrutura Dieta.
 *
 * @param arquivo Nome do arquivo (TSV|bin) a ser lido.
 * @param dieta Ponteiro para a estrutura Dieta onde os dados serão armazenados.
 */
void ficheiro_tsv_2(char *arquivo, Dieta *dieta)
{
    if (strcmp(arquivo, "dietas.tsv") == 0) //**< Compara as strings, caso sejam iguais entra neste loop */
    {
        FILE *ficheiro = fopen(arquivo, "r"); //**< Abre o ficheiro em modo leitura, como o nome ficheiro. */
        ...
        for (int i = 0; i < max; i++)
        {
            if (fgets(linha, sizeof(linha), ficheiro) == NULL) //**< Verifica se as linhas do arquivo CSV estão vazias. */
            {
                break; //**< Sai do loop se não houver mais linhas para ler. */
            }

            /**
             * @brief Extrai dados da linha formatada do arquivo TSV e os armazena na estrutura Dieta.
            ...
            sscanf(linha, "%d\t%d-%d-%d\t%99[^\t]\t%[^\t]\t%d ", &dieta->n_cliente[i], &dieta->dia[i], &dieta->mes[i], &dieta->ano[i], dieta->refeicao[i], dieta->alimento[i], &dieta->calorias[i]);
    ...
    else if (strcmp(arquivo, "dietas_tsv.bin") == 0) //**< Compara as strings, caso sejam iguais entra neste loop */
    {
        FILE *ficheiro = fopen(arquivo, "rb"); //**< Abre o ficheiro em modo leitura (binaria), com o nome ficheiro. */
...
/**
 * @brief Lê dados de um arquivo TSV (tab-separated values) e .bin e armazena os na estrutura Plano.
 *
 * @param arquivo Nome do arquivo (TSV|bin) a ser lido.
 * @param dieta Ponteiro para a estrutura Plano onde os dados serão armazenados.
 */
void ficheiro_tsv_3(char *arquivo, Plano *plano)
{
    if (strcmp(arquivo, "planos.tsv") == 0) //**< Compara as strings, caso sejam iguais entra neste loop */
    {
        FILE *ficheiro = fopen(arquivo, "r"); //**< Abre o ficheiro em modo leitura, como o nome ficheiro. */
    ...
    else if (strcmp(arquivo, "planos_tsv.bin") == 0) //**< Verifica se as linhas do arquivo CSV estão vazias. */
    {
        FILE *ficheiro = fopen(arquivo, "rb"); //**< Abre o ficheiro em modo leitura (binaria), com o nome ficheiro. */
...
/**
 * @brief Está função é responsável por listar um cliente existente no plano, a escolha do utilizador para ficheiros TSV.
 *
...
/**
 * @brief Calcula a média de calorias consumidas por cliente num período de tempo para ficheiros TSV.
 *
...
void calcmedia_tsv(Dados *dados, Dieta *dieta, int diainicio, int mesinicio, int diafim, int mesfim)
{
    ...
    for (i = 0; i < 6; i++)
    {
        if ((mesinicio < dieta->mes[i] && dieta->mes[i] < mesfim) || //**< Válida o período  */
...
/**
 * @brief Tabela com todas as informações do plano nutricional para ficheiros TSV.
 *
...
void tabela_tsv(Dados *dados, Dieta *dieta, Plano *plano)
{
    ...
    for (int i = 0; i < 6; i++)
    {
        ...
        if (dieta->n_cliente[i] == 2) //**< Verifica o cliente */
        {
            ...
            if (strcmp(dieta->refeicao[i], "jantar") == 0) //**< Verifica a refeição */
            {
                cal_jantar2 = cal_jantar2 + dieta->calorias[i];
            }
        }
